...
if __name__ == '__main__':
    ...
    img_idx = '1935'
    image_path = '/home/lsa/Shared/data/EORSSD/test/image/'
    gt_path = '/home/lsa/Shared/data/EORSSD/test/gt/'
    image = image_path + f'{img_idx}.jpg'
    gt = gt_path + f'{img_idx}.png'

    image = Image.open(image)
    gt = pil2tensor(Image.open(gt)).squeeze()
    img = preprocess(image)
    img = img.cuda().unsqueeze(0)  # 因为在batchnorm会检查类型，必须是4D

    model = BASNet(3, 1).cuda()
    model.eval()
    do, d1, d2, d3, d4, d5, d6, db = model(img)
    s1 = tensor2pil(d1.squeeze(0))
    s1.show()
